...
def test_compute_sdhc_v2():
    def compute_sdhc(folder: Path, restart_file: Path) -> SdhcResult:
        compact_velocities_file = folder.joinpath("small.simu.vels.dat.compact")
        frequency_window_width = 0.5e12

        force_constant_file_prefix = folder.joinpath("force_constants")

        Kij, ids_L, ids_R = load_force_constants(folder=force_constant_file_prefix)
    ...
    result = compute_sdhc(folder=folder, restart_file=restart_file)
    assert result.oms_fft is not None
    assert result.SHC_smooth is not None
